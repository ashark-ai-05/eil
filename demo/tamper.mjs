 ...
 *
 *  3. Without `demo/PTR-401.reqs.json` this script falls back to the minimal
 *     unit-test body, which has no clarification, no citation and no sign-off —
 *     so tampers 3, 4 and 5 are SKIPPED, and those include the two the narration
 *     calls the point of the phase. A partial run therefore EXITS NON-ZERO and
 *     says so in words. Rehearsing by exit code must never report success for a
 *     drill that did not demonstrate CLARIFY-005 and GATE-006.
 */
...
// to tamper with. A skip is never a pass: it is counted, reported separately AND
// exits non-zero, because "we did not test that" and "that held" are different
// claims, and a presenter rehearsing by exit code must be told which happened.
...
if (!existsSync(source)) {
  ...
  // On stdout as well as stderr: a presenter watching a projector does not see
  // stderr, and this is the difference between a drill and a partial drill.
  const notice = [
    ...
    `Generate the real artefact with:  ${GENERATE_CMD}`,
  ];
  console.error(notice.join("\n"));
  console.log("");
  for (const l of notice) console.log(`  ${RED(l)}`);
...
if (skipped.length > 0) {
  // A skip asserted nothing, so it must never be summed into the pass count —
  // and it must not exit 0 either. "3 of 6 refused" with a zero status reads as
  // success to anyone rehearsing by exit code, and the tampers that go missing
  // are CLARIFY-005 and GATE-006: the two the narration calls the point of the
  // phase. A drill that did not demonstrate them did not demonstrate the phase.
  console.log(
    `  ${RED(`${skipped.length} of ${results.length} NOT RUN — tampers ${skipped.map((r) => r.t.n).join(", ")}, asserting nothing:`)}`,
  );
  for (const r of skipped) {
    console.log(`  ${RED(`  ${r.t.n}. ${r.t.title} (${r.t.expect.join(", ")}) — ${r.note}`)}`);
  }
  console.log("");
  console.log(`  ${RED("THIS RUN WAS NOT A FULL DEMONSTRATION.")}`);
  if (fellBack) {
    console.log(`  ${RED(`Missing:   ${DEFAULT_SOURCE}`)}`);
    console.log(`  ${RED(`Create it: ${GENERATE_CMD}`)}`);
    console.log(
      `  ${RED("Then re-run this drill. Until then it cannot show CLARIFY-005 or GATE-006.")}`,
    );
  } else {
    console.log(
      `  ${RED(`The artefact ${source} carries nothing to tamper for those checks.`)}`,
    );
    console.log(`  ${RED(`Run the drill against ${DEFAULT_SOURCE}, generated by:  ${GENERATE_CMD}`)}`);
  }
  console.log("");
  process.exit(1);
}
